...
int msgq_id_sm, msgq_id,msgq_id_so,msgq_id_ds;
...
int main(){
    ...
    my_ks = fill_storage_shm(key_id_shm, gc_id_shm, mp_id_shm, msgq_id,msgq_id_so,msgq_id_sm,msgq_id_ds,sem_sync_round,sem_set_tx);
    ...
    while(1){
        ...
        aspetta = create_new_taxi(my_mp,maps,taxi_pid,taxi_list,key_id_shm,taxi_list_pos,position_taxi,array_id_taxi,sem_sync_round);
        ...
        while(x < 2 && inve > 0 ){
            /*NOTA devo implementare un controllo che non ci siano posizioni uguali*/
        ...
        while(i>0){
            ...
            if(mexRcv.msgc[2] == -1){
                ...
                printf("ex_id=%d ex_pos=%d \n",ex_id,ex_pos);
                maps[ex_pos].num_taxi = 0;
                    if(ex_id != -1){
                        printf("non sono arrivato all source \n");
                        kill(taxi_list[ex_id].my_pid, SIGTERM);                                    
                            sem = 0;
                            srand(time(NULL));
                                while(sem<1){
        ...
        while(j>0){
            if((msgrcv(my_ks->msgq_id_ds, &mexRcv, sizeof(mexRcv)-sizeof(long), mexRcv.type,0))==-1){
                TEST_ERROR;
            }
            if(mexRcv.msgc[2] == -1){
